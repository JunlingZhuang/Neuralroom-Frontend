...
const tReceiversValue = gEdges.map((edge) => {
    if (typeof edge.target === 'number') {
        return edge.target;
    }
    return edge.target.index;
});
const tReceivers = wrapVal("receivers", "int32", tReceiversValue);

const tSendersValue = gEdges.map((edge) => {
    if (typeof edge.source === 'number') {
        return edge.source;
    }
    return edge.source.index;
});
...
export function tensorsToGraph(tensors) {
  const [tNodes, tEdges, tReceivers, tSenders, tGlobals, tNumNodes, tNumEdges] =
    tensors;
