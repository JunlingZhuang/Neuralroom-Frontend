...
export class Graph {
  ...
  constructor(
    ...
    private molEditedCallback: (d) => void
  ) {
  ...
  private async init() {
    ...
    const bb = (svg.node() as HTMLElement).getBoundingClientRect();
    this.simulation = d3
      .forceSimulation()
      .force(
        "link",
  ...
  private updateSelectionsToMatchData() {
    ...
    this.edges = this.edgesHolder
      ...
      .on("mouseenter", function (event, d) {
        that.mouseoverEdge(this, event, d);
      })
      .on("mouseleave", function (d) {
        that.mouseoutEdge(this, d);
      })
      .on("click", (event, d) => this.clickEdge(event, d));
    ...
    this.nodes = this.nodesHolder
      ...
      .call(
        d3
          .drag()
          .on("start", (event, d) => this.dragstartedNode(event, d))
          .on("drag", (event, d) => this.draggedNode(event, d))
          .on("end", (event, d) => this.dragendedNode(event, d))
      )
      .on("mouseover", function (event, d) {
        that.mouseoverNode(this, event, d);
      })
      .on("mouseleave", function (event, d) {
        that.mouseoutNode(this, event, d);
      })
      .on("click", (event) => event.stopPropagation()); // 直接使用传递的event对象
  ...
  private render() {
    ...
    selectLines("outline").attr(
      "stroke-width",
      (d) => [0, dblW, tripleW, 0][this.oneHotIdx(d)]
    );
    ...
    selectLines("white")
      ...
      .attr(
        "stroke-width",
        (d) => [0, dblW - lineW * 2, tripleW - lineW * 2, 0][this.oneHotIdx(d)]
      );
    ...
    this.nodes
      ...
      .select("circle")
      .attr("r", 7)
      .attr("opacity", (d) => (d.hidden ? 0 : d.adding ? 0.5 : 1))
  ...
  private dragstartedNode(event, d) {
    this.simulation.stop();
  ...
  private dragendedNode(event, d) {
    ...
    if (this.mode === Mode.Add) {
      ...
      const wasClick =
        Math.abs(d.x - event.x) < 20 && Math.abs(d.y - event.y) < 20;
      if (wasClick) {
        ...
        if (this.hoverNode && this.hoverNode !== d) {
          ...
          this.lastNode().initX = event.x;
          this.lastNode().initY = event.y;
        }
        d.outline = undefined;
  ...
  private mouseoverNode(sel, event, d) {
    d.outline = true;
    this.hoverNode = d;
    ...
    if (!d.hasClose) {
      this.addX(sel, d, false, event);
    }
    this.render();
  }

  private mouseoutNode(sel, evert, d) {
    this.removeX(sel, d);
  ...
  private clickEdge(event, d) {
    event.stopPropagation();
    this.progressOneHotIdx(d);
  }
  private mouseoverEdge(sel, event, d) {
    this.addX(sel, d, true, event);
    d.highlighted = true;
    this.render();
  }
  ...
  private addX(sel, d, edge = false, event) {
    d.hasClose = true;
    const removeNode = () => {
      this.simulation.stop();
      this.data.edges = this.data.edges.filter(
        (e) => e.source != d && e.target != d
      );
    ...
    const close = d3.select(sel).append("g").style("cursor", "default");

    const x = edge
      ? (d.source.x + d.target.x) / 2 + Math.abs(d.source.y - d.target.y) / 4
      : 7;
    const y = edge
      ? (d.source.y + d.target.y) / 2 + Math.abs(d.source.x - d.target.x) / 4
      : -7;
  ...
  private reverseEdge(elt) {
    return this.data.edges.find(
      (e) => e.source == elt.target && e.target == elt.source
    );
  }
